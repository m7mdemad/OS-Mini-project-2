...
class Disk_Process{
    ...
    queue<struct stream> operations; // all operations recieved are added to the queue on recieving them
...
public:
    ...
    void send(long, char*);             /* send data to kernel 
                                        ...
                                        */
    void recieve();                     // recieve data from kernel
...
Disk_Process::Disk_Process(){
    for(int i=0;i<10;i++) strcpy(slots[i].mtext,"");       // intialize all slots as empty arrays
...
int Disk_Process::Add(char* newData){
    if(free_count == 0) return 1;                         // slots are full
...
int Disk_Process::Delete(char slotNo){
    ...
    if(slots[number_of_slot].mtext == "") return 3;       // slot is already empty
...
void Disk_Process::update_Operations(){
    ...
    time_needed.front() -= 1;
    if(time_needed.front() == 0){                                       // first operation time is finished
        struct stream op = operations.front();
        operations.pop();                                       // remove it from operations running
        time_needed.pop(); 
        if(op.command == "ADD") op.state_count = Add(op.data);  // perform add
        else op.state_count = Delete(op.data[0]);               // perform delete 
...
void Disk_Process::recieve(){
    struct stream to_recieve;
    int rec_val = msgrcv(downStream, &to_recieve, sizeof(to_recieve.data), 0, !IPC_NOWAIT);  
  
  	if(rec_val == -1){perror("Error in receive at disk");}
    addOperation(to_recieve);                   // add the new operation to the operations queue
}

void Disk_Process::addOperation(struct stream newOp){
     operations.push(newOp); 
     int time =newOp.clks;                      // no. of clks needed
     if(newOp.command == "ADD") time += 3;      // add latency
     else time+=1;                              // delete latency
